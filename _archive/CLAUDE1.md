...
**Statut : EN ATTENTE (démarrer seulement quand Phase 0 est 100% validée)**
...
### ✅ PHASE 3 — WebSocket + pages HTML cuisine & display
**Statut : TERMINÉE**

- [x] WebSocket `/ws/events` en place
- [x] Page cuisine servie par Fastify (`/cuisine`)
- [x] Page display servie par Fastify (`/display`)
- [x] Broadcast temps réel vers tous les clients connectés

---

### ⏳ PHASE 4 — VPS cloud, sync, click & collect
**Statut : EN ATTENTE (démarrer seulement quand Phase 3 est 100% validée)**

> Voir ARCHITECTURE.md section 6 et 12 pour le détail complet.

#### Étape 4.1 — Setup VPS backend
- [ ] Installer PostgreSQL sur le VPS
- [ ] Créer le schéma de la base cloud (commandes, produits, catégories, comptes)
- [ ] Créer un projet Fastify séparé sur le VPS
- [ ] Réutiliser `packages/shared/` pour les types

#### Étape 4.2 — Champ `published` sur les produits
- [ ] Ajouter colonne `published` (boolean, défaut false) dans SQLite
- [ ] Ajouter toggle dans la gestion des produits sur le POS
- [ ] Le SyncService ne sync que les produits avec `published: true`

#### Étape 4.3 — SyncService produits POS → VPS
- [ ] Sync des produits publiés vers PostgreSQL VPS
- [ ] Sync des catégories vers PostgreSQL VPS
- [ ] Retry automatique si réseau down

#### Étape 4.4 — SyncService commandes POS → VPS
- [ ] Ajouter `syncStatus` sur les commandes SQLite (pending / synced / error)
- [ ] Sync async après chaque paiement
- [ ] Retry automatique si réseau down

#### Étape 4.5 — Routes VPS Fastify
- [ ] `GET  /api/products?published=true` → pour le site vitrine
- [ ] `GET  /api/orders` → pour le panel admin
- [ ] `POST /api/orders` → pour le click & collect
- [ ] `GET  /api/orders/pending` → long polling pour le POS

#### Étape 4.6 — Long polling click & collect sur le POS
- [ ] POS poll `GET /api/orders/pending` (VPS) toutes les 30s
- [ ] Nouvelle commande click & collect → apparaît sur le POS comme commande normale
- [ ] Statut "prêt" sync vers VPS → notification client

#### Étape 4.7 — Panel admin (interface web sur VPS)
- [ ] Page web simple sur le VPS
- [ ] Vue globale des commandes en temps réel
- [ ] Stats basiques (CA, nb commandes, produits populaires)
- [ ] Compte propriétaire séparé (pas lié aux users POS)

#### Étape 4.8 — Tests unitaires Vitest (archi stable)
...
#### Tests Phase 4
- [ ] Créer `tests/phase4/sync.curl.sh` — vérifier sync POS → VPS
- [ ] Créer `tests/phase4/clickcollect.curl.sh` — simuler commande en ligne
- [ ] Créer `tests/phase4/checklist.md` — panel admin, offline, produits publiés
- [ ] Lancer les tests unitaires Vitest

#### Validation Phase 4
- [ ] Produits publiés visibles sur le site vitrine automatiquement
- [ ] Produits brouillon jamais exposés sur le site
- [ ] Commandes payées synchronisées vers VPS
- [ ] Click & collect apparaît sur le POS en quasi temps réel
...
**Dernière session :** Définition de l'architecture, du plan d'action et création de ce fichier
**Phase active :** Phase 0 — Analyse complète du projet
**Prochaine tâche :** Lire et analyser tous les fichiers listés dans la Phase 0, un par un
**Blockers :** Aucun

---

## 📝 Conclusions d'analyse
*(Section vide — à remplir après la Phase 0, avec validation du développeur)*
